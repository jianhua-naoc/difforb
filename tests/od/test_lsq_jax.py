...
from difforb.od.dc.lsq import LSQTermination, LeastSquares, RobustLeastSquares
...
def test_lm_reports_accepted_step_limits(max_iter):
    ...
    result = LeastSquares(max_iter=max_iter).solve(
        jnp.asarray([2.]), jnp.ones(1, bool), residual, linearize,
    )
...
def test_lm_stops_after_damping_failure():
    ...
    result = solver.solve(
        jnp.zeros(1), jnp.ones(1, bool), residual, linearize,
    )
...
def test_statistically_small_correction_can_have_large_physical_change():
    ...
    result = LeastSquares(max_iter=1).solve(jnp.zeros(7), jnp.ones(7, bool), residual, linearize)
...
def test_large_damping_cannot_satisfy_correction_criterion():
    ...
    result = solver.solve(jnp.zeros(1), jnp.ones(1, bool), residual, linearize)
...
def test_rms_stagnation_is_an_independent_success_reason_with_logging():
    ...
    solver = LeastSquares(max_iter=10)
    solver.damping_init = 1e6
    events = []
    result = solver.solve(jnp.ones(1), jnp.ones(2, bool), residual, linearize,
                          event_handler=events.append, log_detail="trial")
    assert result.converged
    assert result.termination_reason == "rms_stagnated"
    assert result.iter_num == 6
    assert result.params[0] > .9  # delnor remains far above its threshold.
    assert events[-1].data["reason"] == "rms_stagnated"
    assert events[-1].data["converged"]
...
def test_stagnation_counter_resets_on_improvement_and_ignores_rejected_trials(reject):
    ...
    linearize = lambda x: (jnp.ones((1, 1)), residual(x), jnp.empty((0, 2, 2)), jnp.ones(1))
    x, mask, scale = jnp.zeros(1), jnp.ones(1, bool), jnp.ones(1)
    options = {"max_iter": 20, "max_damping_iter": 10, "damping_init": 1e-3}
    state = initialize_lsq(x, mask, linearize, scale, options)._replace(stagnant_steps=jnp.asarray(5))
    state = advance_lsq(state, mask, residual, linearize, scale, options)
    assert state.stagnant_steps == (5 if reject else 0)
    assert state.termination_code == LSQTermination.running


@pytest.mark.parametrize("exponent,reason", [(10., "rms_stagnated"), (1000., "rms_increasing")])
def test_rms_increase_uses_refreshed_weights_and_distinguishes_failure(exponent, reason):
    ...
    accepted = [e for e in events if e.event == "lsq_step_accepted"]
    np.testing.assert_allclose(accepted[-1].data["normalized_residual_rms_after"], result.normalized_residual_rms)
    np.testing.assert_allclose(accepted[0].data["normalized_residual_rms_after"],
                               accepted[1].data["normalized_residual_rms_before"])
    failures = [e for e in events if e.event == "lsq_failed"]
    assert bool(failures) == (reason == "rms_increasing")


@pytest.mark.parametrize("name", ["tol", "rms_tol", "stagnation_steps"])
def test_convergence_settings_are_not_constructor_options(name):
...
def test_inner_solve_jit_and_vmap_have_no_host_callbacks():
    ...
    solve = jax.jit(lambda x: solver.solve(x, jnp.ones(1, bool), residual, linearize))
...
def test_robust_loop_is_compiled_and_refits_changed_mask(max_iters):
    ...
    solve = jax.jit(lambda x: solver.solve(x, policy, residual, linearize))
    x0 = jnp.asarray([10.])
...
def test_dynamic_weights_nonzero_residual_fixed_point():
    ...
    result = solver.solve(jnp.zeros(1), jnp.ones(2, bool), residual, linearize)
...
def test_parameter_scaling_preserves_small_model_parameters():
    ...
    result = LeastSquares(max_iter=50).solve(
        jnp.zeros(7), jnp.ones(8, bool), residual, linearize, param_scale=scales,
    )
    assert result.converged
...
def test_initial_convergence_limits_and_nonfinite_models(solver_jit):
    ...
    exact = solver.solve(jnp.ones(1), jnp.ones(1, bool), residual, linearize)
    limited = solver.solve(jnp.asarray([.1]), jnp.ones(1, bool), residual, linearize)
    invalid = solver.solve(jnp.asarray([jnp.nan]), jnp.ones(1, bool), residual, linearize)
...
def test_nonfinite_trial_is_rejected_without_changing_parameters(solver_jit):
    residual = lambda x: jnp.where(x > 0., jnp.nan, x-1.)
    linearize = lambda x: (jnp.ones((1, 1)), residual(x), jnp.empty((0, 2, 2)), jnp.ones(1))
    result = LeastSquares(solver_jit=solver_jit).solve(jnp.zeros(1), jnp.ones(1, bool), residual, linearize)
    assert result.termination_code == LSQTermination.damping_failed
    assert not result.converged
    assert result.iter_num == 0
    np.testing.assert_array_equal(result.params, [0.])
...
@pytest.mark.parametrize("solver_jit", [False, True])
def test_zero_iteration_budget_with_logging(solver_jit):
    residual, linearize = nonlinear_problem()
    events = []
    result = LeastSquares(max_iter=0, solver_jit=solver_jit).solve(
        jnp.asarray([.1]), jnp.ones(1, bool), residual, linearize,
        event_handler=events.append, log_detail="trial",
    )
    assert result.termination_reason == "max_iter_reached"
    assert result.iter_num == 0
    np.testing.assert_array_equal(result.params, [.1])
    expected = ["lsq_done"] if solver_jit else ["lsq_start", "lsq_done"]
...
def test_singular_fit_skips_rejection_with_unavailable_metrics(solver_jit):
    ...
    result = RobustLeastSquares(LeastSquares(solver_jit=solver_jit)).solve(jnp.zeros(2), outlier_policy(), residual, linearize)
...
def test_logging_preserves_numerical_results_and_final_refit():
    ...
    quiet = solver.solve(jnp.asarray([10.]), outlier_policy(), residual, linearize, log_detail="quiet")
    events = []
    logged = solver.solve(jnp.asarray([10.]), outlier_policy(), residual, linearize,
                           event_handler=events.append, log_detail="trial")
    ...
    assert len([e for e in events if e.event == "lsq_done"]) == 2
    assert len([e for e in events if e.event == "outlier_update"]) == 1
    assert events[-1].event == "outlier_done"
    assert events[-1].data["stop_reason"] == "max_iterations_reached"
    assert events[-1].data["inlier_count"] == 3
...
def test_batched_stagnation_and_correction_stop_independently():
    ...
    solver.damping_init = 1e6
    solve = jax.jit(jax.vmap(lambda x: solver.solve(x, jnp.ones(2, bool), residual, linearize)))
    ...
    np.testing.assert_array_equal(result.iter_num, [1, 6])
...
def test_robust_refits_use_identical_convergence_settings():
    ...
    x0 = jnp.asarray([10.])
    first = solver.solve(x0, jnp.ones(4, bool), residual, linearize)
    final_mask = jnp.asarray([True, True, True, False])
    second = solver.solve(first.params, final_mask, residual, linearize)
    robust = RobustLeastSquares(solver).solve(x0, outlier_policy(), residual, linearize)
...
def test_solver_jit_modes_preserve_fit(robust):
    ...
    for compiled in [False, True]:
        ...
        events = []
        result = solver.solve(x0, selection, residual, linearize, event_handler=events.append, log_detail="trial")
        results.append(result)
        histories.append([e.event for e in events])
    ...
    assert "lsq_step_accepted" in histories[0]
    assert "lsq_step_accepted" not in histories[1]
    assert "lsq_done" in histories[1]
...
def test_python_solver_calls_compiled_steps_without_changing_jit_context(monkeypatch):
    ...
    result = LeastSquares().solve(x0, jnp.ones(1, bool), residual, linearize)
    assert result.converged
...
def test_solver_jit_context_restored_after_model_failure():
    def fail(x):
        assert not jax.config.jax_disable_jit
        raise RuntimeError("model failed")

    with pytest.raises(RuntimeError, match="model failed"):
        LeastSquares().solve(jnp.zeros(1), jnp.ones(1, bool), fail, fail)
    assert not jax.config.jax_disable_jit
...
def test_python_loops_bypass_complete_compiled_driver(monkeypatch, robust):
    ...
    inner = LeastSquares()
    if robust:
        result = RobustLeastSquares(inner).solve(jnp.asarray([10.]), outlier_policy(), residual, linearize)
        expected_fits = 2
        fit = result.lsq_result
        np.testing.assert_array_equal(result.rej_result.flat_inlier_mask, [True, True, True, False])
    else:
        fit = inner.solve(jnp.asarray([10.]), jnp.ones(4, bool), residual, linearize)
        expected_fits = 1
